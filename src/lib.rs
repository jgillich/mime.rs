    ...
    fn test_mime_from_str() {
        ...
        assert_eq!(FromStr::from_str("text/plain; charset=utf-8; foo=bar"),
            Some(Mime(TopLevel::Text, SubLevel::Plain, vec![(Attr::Charset, Value::Utf8),
                                        (Attr::AttrExt("foo".to_string()), Value::ValueExt("bar".to_string())) ])));
    }


    #[bench]
    fn bench_show(b: &mut Bencher) {
        let mime = Mime(TopLevel::Text, SubLevel::Plain, vec![(Attr::Charset, Value::Utf8), (Attr::AttrExt("foo".to_string()), Value::ValueExt("bar".to_string()))]);
